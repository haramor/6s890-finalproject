...
from models.simple_game_theoretic_loss import SimpleGameTheoreticLoss, LabelSmoothedCE
    ...
    for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        ...
        if (i + 1) % config.BATCHES_PER_STEP == 0:
            ...
            if step % config.PRINT_FREQUENCY == 0:
                ...
                writer.add_scalar("train/entropy", kl_losses.val, step)  # Entropy regularization
                ...
                print(f"Step {step}/{config.N_STEPS} | "
                      ...
                      f"Entropy: {kl_losses.val:.4f} | "  # Actually entropy, not KL
...
def main():
    ...
    # Create criterion
    if config.USE_GT_REGULARIZATION:
        criterion = SimpleGameTheoreticLoss(
        ...
        print("Using simple game-theoretic entropy regularization")
        print(f"  GT Weight: {config.GT_WEIGHT}")
        print("  Approach: Maximum entropy principle - encourages exploration")
    else:
