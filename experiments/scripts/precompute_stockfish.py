...
def precompute_stockfish_evaluations(
    ...
    try:
        ...
        print("Attempting to load h5 file directly...")
        
        # Fallback: load h5 file directly
        h5_path = Path(data_dir) / h5_file
        with h5py.File(h5_path, 'r') as f:
            print(f"H5 file keys: {list(f.keys())}")
            # This is a simplified approach - adjust based on your h5 structure
            encoded_data = f['encoded_data'][:]
            print(f"✓ Loaded {len(encoded_data)} encoded positions")
    ...
    # We'll use FEN strings as keys since they uniquely identify positions
    for i in tqdm(range(len(dataset)), desc="Evaluating positions"):
        try:
            ...
            # Skip if already computed (in case of duplicate positions)
            ...
            if dist:  # Only store non-empty distributions
